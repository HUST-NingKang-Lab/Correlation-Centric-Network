/* 
 * File:   main.cpp
 * Author: Pengshuo Yang, Chongyang Tan, Maozhen Han, Lin Cheng, Xuefeng Cui, Kang Ning
 * The Correlation-centric network are constructed based on  the paper by T.S.Evans and R.Lambiotte,
 * correlationcenticnetworks, Link Partitions and Overlapping Communities,
 * Phys.Rev.E <b>80</b> (2009) 016105 [arXiv:0903.2181].
 * Created on 14 August 2018, 12:25
 */
...
void 
printCorrelationcentricnetworkTypes(ostream& os){
 os << "--- Correlation-centric network Types " <<endl;
...
usage(char *prog_name, char *more, ostream& os) {
  ...
  os << "-t n\tcreate correlation centic network of type 0<=n<=3. Default is 2." << endl;
  os << "-w\tread the graph as a weighted one. Otherwise graph is unweighted." << endl;
  os << "-h\tshow this usage message." << endl;
  printCorrelationcentricnetworkTypes(os);
  printFileFormats(os);
}
...
/*
 * correlationcenticnetwork Types:-
 ...
 */
void
makeCorrelationcentricnetwork(TseGraph& tg, TseGraph& lg, int newType, bool infoOn){

        int type=2;
        if ((newType>-1)&&(newType<4)) type = newType;
        else  {cout << "*** makeCorrelationcentricnetwork needs type to be between 0 and 3, was given " << newType << endl; exit(0);}

        cout << "--- Making correlationcenticnetwork of type  " << newType << endl; 
        ...
        if (infoOn) cout << "Predicting " <<lgMaximumVertices << " vertices and " << lgMaximumStubs << " stubs in correlation centic network " << endl;
...
//                }
                ...
                for (int ni = 0; ni < tg.getVertexDegree(v); ni++) {
                    ...
                    for (int no = ni+noselfloops; no < tg.getVertexDegree(v); no++) {
                      ...
                      // use edge index=stub/2 for the vertex indices in the correlation centic network
                      if (lgweighted) {
            ...
            time(&time_end);
            if (infoOn )cout << "\nFinished makeCorrelationcentricnetwork, time taken "<< (time_end-time_begin) << "s" << endl;
...
int main(int argc, char **argv) {
  ...
  cout << "  Output file " << outfile << ", correlation centic network type "<< lgType << endl;
  ...
  // make correlation centic network
  time(&time_begin);
  TseGraph lg;
  makeCorrelationcentricnetwork(tg,lg,lgType, infoOn);
  time(&time_end);
  display_time("finished making correlation centic network");
  cout <<  " time taken to make correlation centic network " << (time_end-time_begin) << "s" << endl;
  if (lg.getNumberVertices()<20) lg.write();
   cout << "correlation centic network: "
  ...
  //output correlation centic network file
  time(&time_begin);
  lg.write(outfile);
  time(&time_end);
  display_time("finished making correlation centic network file ");
  cout <<  " time taken to write correlation centic network to file "<< outfile << " " << (time_end-time_begin) << "s"  << endl;
